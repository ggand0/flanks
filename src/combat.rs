...
pub fn process_deaths(
    ...
    // job listed every man this can apply to (the countdowns that ran
    // out, the living men near their edge), so the sweep visits those
    // positions in ascending order and nobody else. A swap-remove
    // brings the last man into the hole, and he is examined in the
    // same place, exactly as a scan of every position did; the columns
    // end up in the same order.
    let removable = |units: &Units, i: usize| -> (bool, bool) {
    ...
    for &c in &pipeline.sweep_candidates {
        let c = c as usize;
        if c >= units.len() {
            break;
        }
        let i = c;
        loop {
            let (dead, fled) = removable(&units, i);
            if !(dead || fled) {
                break;
            }
            let team = units.team[i] as usize;
